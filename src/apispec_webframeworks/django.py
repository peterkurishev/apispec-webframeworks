"""Django plugin. Includes a path helper that allows you to pass a view
function to `path`. Inspects URL rules and view docstrings.

Passing a view function::

    from flask import Django TODO
...
class DjangoPlugin(BasePlugin):
    """APISpec plugin for Django"""

    @staticmethod
    def flaskpath2openapi(path):
        """Convert a Django URL rule to an OpenAPI-compliant path.

        :param str path: Django path template.
        """
    ...
    def path_helper(self, operations, *, view, app=None, **kwargs):
        """Path helper that allows passing a Django view function."""
